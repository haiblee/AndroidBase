...
import com.haiblee.base.log.XLog;
...
final class CocoSQLiteOpenHelper extends SQLiteOpenHelper {
    ...
    public void onOpen(SQLiteDatabase db) {
        super.onOpen(db);
        XLog.d(TAG,"onOpen：db hashCode = %s,dbPath = %s",db.hashCode(),db.getPath());
    }

    @Override
    public void onConfigure(SQLiteDatabase db) {
        super.onConfigure(db);
        XLog.d(TAG,"onConfigure：db hashCode = %s,dbPath = %s",db.hashCode(),db.getPath());
    ...
    public final void onCreate(SQLiteDatabase db) {
        synchronized (mDatabase){
            if(mDatabase.getFlagValue(AbstractDatabase.FLAG_CREATED)){
                XLog.w(TAG, String.format("onCreate：the database(%s,%s) FLAG_CREATED value is true,create completed,don't need to repeat",mDatabase.hashCode(),mDatabase.databaseName()));
                return;
            }
            SparseArray<ITable[]> arrays = mDatabase.staticTables();
            XLog.d(TAG, String.format("onCreate: mDatabase(%s,%s),table arrays size = %s",mDatabase.hashCode(),mDatabase.databaseName(),(arrays == null ? "null" : arrays.size())));
            if(arrays != null){
    ...
    public final void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        synchronized (mDatabase){
            if(mDatabase.getFlagValue(AbstractDatabase.FLAG_UPGRADED)){
                XLog.w(TAG, String.format("onUpgrade：the database(%s,%s) FLAG_UPGRADED value is true,upgrade completed,don't need to repeat",mDatabase.hashCode(),mDatabase.databaseName()));
                return;
            }
            SparseArray<ITable[]> staticArrays = mDatabase.staticTables();
            XLog.d(TAG, String.format("onUpgrade: mDatabase(%s,%s),table staticArrays size = %s",mDatabase.hashCode(),mDatabase.databaseName(),(staticArrays == null ? "null" : staticArrays.size())));

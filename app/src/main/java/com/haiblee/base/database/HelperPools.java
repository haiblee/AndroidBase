...
class HelperPools {
    ...
    public CocoSQLiteOpenHelper obtain() {
        ...
        if(helper == null && mCounter.incrementAndGet() <= mMaxSize){
            ...
            XLog.d(TAG,"HelperPools(db:%s) obtain() , peek return null,new instance,mCounter = %s,mMaxSize = %s",database.hashCode(),mCounter.get(),mMaxSize);
            return helper;
        }else{
            try {
                return mQueue.take();
            ...
            XLog.d(TAG, String.format("HelperPools(db:%s) obtain() , take Exception new instance, mCounter = %s,mMaxSize = %s",database.hashCode(),count,mMaxSize));
    ...
    public boolean recycle(CocoSQLiteOpenHelper helper) {
        ...
        if(isDestroy){
            try{
                XLog.d(TAG,"recycle()：already destroyed,call helper close()");
                helper.close();
                return true;
            }catch (Exception e){
                e.printStackTrace();
        ...
        if(!success){
            ...
            XLog.w(TAG, String.format("HelperPools(db:%s) recycle queue offer return false ,maybe queue full! mQueue size = %s,mMaxSize = %s",database.databaseName(),mQueue.size(),mMaxSize));
        }
        return success;
    }
    ...
    public synchronized void destroy(){
        XLog.d(TAG,"destroy()：pools size = %s",mQueue.size());
        while (true){

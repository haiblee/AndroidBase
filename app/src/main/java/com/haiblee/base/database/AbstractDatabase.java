...
public abstract class AbstractDatabase implements IDatabaseExecutor {
    ...
    private void logErrorMessage(String table, String method, Exception e, String msg) {
        XLog.e(TAG, String.format("%s[%s]：%s->[%s]，Exception：%s[%s]，msg = %s",mExecutor.getClass().getSimpleName(),databaseName(),table, method, e.getClass(), e.getMessage(), msg));
    }

    private void logDurationMessage(String table, String method, long duration){
        XLog.d(TAG, String.format("%s[%s]：[%s]->[%s]，duration = %sms",mExecutor.getClass().getSimpleName(),databaseName(),table,method,duration));
    }
}
